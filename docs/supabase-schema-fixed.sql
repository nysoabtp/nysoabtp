...
DO $ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_chantier_devis') THEN ALTER TABLE chantiers ADD CONSTRAINT uq_chantier_devis UNIQUE (devis_id); END IF; END $;
