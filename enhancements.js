...
function reApplyChantierFilter() {
    if (typeof currentChantierFilter !== 'undefined' && currentChantierFilter) {
...
async function buildCrossFilterBar() {
    ...
    const sections = ['journal', 'achats', 'pointage', 'personnel', 'projets', 'logistique', 'devis', 'proformat', 'rapports', 'salaires'];
...
function applyGlobalChantierFilter(chantierRef) {
    ...
    filterTableByChantier('personnel-table', chantierRef);
    filterTableByChantier('projets-table', chantierRef);
    filterTableByChantier('devis-tbody', chantierRef);
    filterTableByChantier('stock-table-body', chantierRef);
    filterTableByChantier('salaires-journaliers-body', chantierRef);
    filterTableByChantier('salaires-mensuels-body', chantierRef);
    filterPointageByChantier(chantierRef);
    filterLogistiqueByChantier(chantierRef);
}
...
function filterLogistiqueByChantier(chantierRef) {
    // Stock table
    const stockBody = document.getElementById('stock-table-body');
    if (stockBody) {
        stockBody.querySelectorAll('tr').forEach(row => {
            if (!chantierRef) { row.style.display = ''; return; }
            row.style.display = row.textContent.toLowerCase().includes(chantierRef.toLowerCase()) ? '' : 'none';
        });
    }
    // Mouvements table
    const mvtBody = document.getElementById('mouvement-table-body');
    if (mvtBody) {
        mvtBody.querySelectorAll('tr').forEach(row => {
            if (!chantierRef) { row.style.display = ''; return; }
            row.style.display = row.textContent.toLowerCase().includes(chantierRef.toLowerCase()) ? '' : 'none';
        });
    }
