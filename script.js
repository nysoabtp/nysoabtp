...
/**
 * Filtre les lignes d'une table par le texte d'une colonne donnée.
 * @param {string} tableId  - id de la <table>
 * @param {string} value    - valeur à comparer (vide = tout montrer)
 * @param {number} colIndex - index de colonne 0-based (contenant le statut)
 */
function filtrerTableParStatut(tableId, value, colIndex) {
    const table = document.getElementById(tableId);
    if (!table) return;
...
// ── Journal ──────────────────────────────────────────────────
function filtrerJournalType(value) {
    ...
    rows.forEach(row => {
        const typeCell = row.cells[5]; // colonne Catégorie
        const dateCell = row.cells[0];
        const typeMatch = !value || (typeCell && typeCell.textContent.trim().toUpperCase().includes(value.toUpperCase()));
        const dateMatch = !dateVal || (dateCell && dateCell.textContent.trim() === new Date(dateVal).toLocaleDateString('fr-FR'));
        row.style.display = (typeMatch && dateMatch) ? '' : 'none';
    });
}

function filtrerJournalDate(value) {
    const rows = document.querySelectorAll('#journal-table tbody tr');
    ...
    rows.forEach(row => {
        ...
        const dateMatch = !value || (dateCell && dateCell.textContent.trim() === new Date(value).toLocaleDateString('fr-FR'));
        const typeMatch = !typeVal || (typeCell && typeCell.textContent.trim().toUpperCase().includes(typeVal.toUpperCase()));
        row.style.display = (dateMatch && typeMatch) ? '' : 'none';
    });
}

// ── Personnel ────────────────────────────────────────────────
function filtrerPersonnelRecherche(value) {
    ...
    rows.forEach(row => {
        const nameCell = row.cells[1]; // Nom
        const deptCell = row.cells[3]; // Chantier/Département
        const nameMatch = !value || (nameCell && nameCell.textContent.toLowerCase().includes(value.toLowerCase()));
        const deptMatch = !deptVal || (deptCell && deptCell.textContent.toLowerCase().includes(deptVal.toLowerCase()));
        row.style.display = (nameMatch && deptMatch) ? '' : 'none';
    });
}

function filtrerPersonnelDept(value) {
    const rows = document.querySelectorAll('#personnel-table tbody tr');
    ...
    rows.forEach(row => {
        ...
        const deptMatch = !value || (deptCell && deptCell.textContent.toLowerCase().includes(value.toLowerCase()));
        const nameMatch = !searchVal || (nameCell && nameCell.textContent.toLowerCase().includes(searchVal.toLowerCase()));
        row.style.display = (deptMatch && nameMatch) ? '' : 'none';
    });
}

console.log('[NYSOA BTP] filtres génériques chargés ✓');
