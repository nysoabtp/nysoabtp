...
async function updateAchatsStats(data) {
  if (!data) {
    const { data: d } = await db.from('achats').select('prix, statut');
    data = d || [];
  }
...
async function addAchat(achat) {
  const { error } = await db.from('achats').insert({
    date_achat:    achat.date || today(),
...
async function deleteAchat(id) {
  ...
  const { error } = await db.from('achats').delete().eq('id', id);
...
async function addPersonnel(emp) {
  const { error } = await db.from('personnel').insert({
    ...
    chantier_code:     emp.chantier_code,
...
async function loadChantiers() {
  const { data, error } = await db.from('chantiers').select('code, nom').order('code');
...
async function updateDashboardStats() {
  ...
  const { count: nbMat } = await db
    .from('materiaux').select('*', { count: 'exact', head: true });
...
async function exportJournalToExcel() {
  const { data, error } = await db.from('journal').select('*').order('date_ecriture');
