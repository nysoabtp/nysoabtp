...
async function ouvrirEditeurDevis(id = null) {
    if (id) {
        ...
        const num = numData || ('DEV-' + new Date().getFullYear() + '-' + String(Math.floor(Math.random() * 900) + 100));
        devisEnCours = {
...
document.addEventListener('DOMContentLoaded', () => {
    // Charger la liste au démarrage si on est sur la section devis
    if (document.getElementById('devis-tbody')) loadDevisTable();
});
