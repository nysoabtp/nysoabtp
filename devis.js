...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        // Attendre que supabase.js ait défini db
...
// ── RECHARGEMENT FORCÉ ────────────────────────────────────────
async function forceReloadDevis() {
    if (typeof showNotification === 'function') showNotification('Chargement des devis…', 'info');
    await loadDevisTable();
}

// ── SEED FORCÉ AMBOHIMANABE (ignore la vérification doublon) ──
async function forceSeedAmbohimanabe() {
    if (typeof showNotification === 'function') showNotification('Import Ambohimanabe en cours…', 'info');
    try {
        // Vérifier si déjà présent
        const { data: existing } = await db.from('devis')
            .select('id').eq('client', 'Mr Herman AMBOHIMANABE').limit(1);
        if (existing && existing.length > 0) {
            if (typeof showNotification === 'function')
                showNotification('Devis Ambohimanabe déjà présent dans la base ✓', 'warning');
            await loadDevisTable();
            return;
        }
        // Forcer l'insertion
        ...
        await loadDevisTable();
        if (typeof showNotification === 'function')
            showNotification('Devis Ambohimanabe importé ✓', 'success');
    } catch(e) {
        console.error('[forceSeedAmbohimanabe]', e);
        if (typeof showNotification === 'function')
            showNotification('Erreur import : ' + (e.message || e), 'error');
    }
}
