...
-- 7. Add policy: RH/Admin can insert/update all personnel (pas de DELETE pour RH)
DROP POLICY IF EXISTS "RH Admin manage personnel" ON personnel;

CREATE POLICY "RH Admin read all personnel" ON personnel
FOR SELECT USING (
    auth.jwt()->'user_metadata'->>'role' IN ('rh', 'admin')
);

CREATE POLICY "RH Admin insert personnel" ON personnel
FOR INSERT WITH CHECK (
    auth.jwt()->'user_metadata'->>'role' IN ('rh', 'admin')
);

CREATE POLICY "RH Admin update personnel" ON personnel
FOR UPDATE USING (
    auth.jwt()->'user_metadata'->>'role' IN ('rh', 'admin')
);

-- Seul admin peut supprimer (désactiver) des employés
CREATE POLICY "Admin delete personnel" ON personnel
FOR DELETE USING (
...
-- RH can insert/read/update salaires (pas de suppression)
DROP POLICY IF EXISTS "RH manage salaires" ON salaires;

CREATE POLICY "RH read salaires" ON salaires
FOR SELECT USING (auth.jwt()->'user_metadata'->>'role' = 'rh');

CREATE POLICY "RH insert salaires" ON salaires
FOR INSERT WITH CHECK (auth.jwt()->'user_metadata'->>'role' = 'rh');

CREATE POLICY "RH update salaires" ON salaires
FOR UPDATE USING (auth.jwt()->'user_metadata'->>'role' = 'rh');
...
-- Guard IS NOT NULL : évite résultat vide silencieux si métadonnée absente
...
-- Admin peut créer et modifier les chantiers (pas de suppression directe)
DROP POLICY IF EXISTS "Admin manage chantiers" ON chantiers;

CREATE POLICY "Admin insert chantiers" ON chantiers
FOR INSERT WITH CHECK (auth.jwt()->'user_metadata'->>'role' = 'admin');

CREATE POLICY "Admin update chantiers" ON chantiers
...
-- Admin peut lire et mettre à jour les validations
DROP POLICY IF EXISTS "Admin manage validations" ON validations;

CREATE POLICY "Admin read validations" ON validations
FOR SELECT USING (auth.jwt()->'user_metadata'->>'role' = 'admin');

CREATE POLICY "Admin update validations" ON validations
FOR UPDATE USING (auth.jwt()->'user_metadata'->>'role' = 'admin');
