...
async function saveAntokaPayment() {
    ...
    try {
        ...
        loadAntoka();
        showToast('Paiement antoka enregistré ✓');
...
async function saveCredit(e) {
    ...
    showToast('Crédit fournisseur ajouté ✓');
}

async function deleteCredit(id) {
    if (!confirm('Supprimer ?')) return;
    await db.from('credits_fournisseurs').delete().eq('id',id);
    loadCredits(); showToast('Supprimé');
}
...
function usePrix(designation, pu) {
    showToast(`${designation} — ${pu.toLocaleString('fr-FR')} Ar copié`);
}
...
async function deleteContrat(id) {
    ...
    loadContrats(); showToast('Supprimé');
}

/* ══════════════════════════════════════════════════════════════
   INITIALISATION — lier les sections aux fonctions de chargement
   ══════════════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', () => {
    // Charger automatiquement quand on navigue vers ces sections
    document.querySelectorAll('.nav-item[data-section]').forEach(item => {
        item.addEventListener('click', () => {
            const sec = item.dataset.section;
            if (sec === 'antoka')    setTimeout(loadAntoka, 100);
            if (sec === 'credits')   setTimeout(loadCredits, 100);
            if (sec === 'caisse')    setTimeout(loadCaisse, 100);
            if (sec === 'catalogue') setTimeout(loadCatalogue, 100);
            if (sec === 'contrats')  setTimeout(loadContrats, 100);
        });
    });
